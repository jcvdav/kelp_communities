...
```{r load data}
length_data <- read.csv(here("data","Peces_KelpForest_2011-2013.csv"), stringsAsFactors = F)
```
...
filter(length_data, transect > 0, year == 2013) %>%
...
indicator_data <- length_data %>% 
  ...
  rename(Abundancia = abundance,
         ...
         Talla = totallength,
         Ano = year) %>% 
  filter(Sitio %in% c("ASA", "BMA", "ERE", "ERO", "ISME", "ITSP", "SMI", "SSI"))
...
indicator_plot <- function(x, ylab = "", order = NULL){
  ...
  left_join(x, order, by = "Sitio") %>% 
    ...
    ggplot(mapping = aes(x = Sitio, y = Indicador, fill = Ano, group = paste(Ano, Sitio))) +
    geom_boxplot(outlier.size = 0) +
    stat_summary(geom = "point", fun.y = "mean", color = "black", position = position_dodge(width = 1)) +
    theme_bw() +
...
D_plot <- indicator_plot((D %>% mutate(Indicador = log10(Indicador))), ylab = "log10 Density (org / transect)", order)

B_plot <- indicator_plot((B %>% mutate(Indicador = log(Indicador))), ylab = "Biomass (Kg / transect)", order)


cowplot::plot_grid(H_plot, S_plot, D_plot, B_plot, ncol = 2)
```
